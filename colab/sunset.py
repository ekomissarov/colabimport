...
def do_calculations_base_values(x):
    ...
    x['conv_agg_full'] = x['events'] + (8 * x['ads']) + (500 * x['ipotek']) + (500 * x['ct']) + (
                8 * x['events_applications']) + (10 * x['events_ss'])
...
def do_calculations_base_values_with_assisted(x):
    ...
    x['A_events_commercial'] = x['total_events_commercial'] + x['total_events_app_commercial'] + x[
        'assisted_conv_phones']
    ...
    x['A_events_applications'] = x['total_applications_re_events'] + x['total_applications_re_events_app'] + x[
        'assisted_conv_reappl']
    ...
    x = x.copy()
    # агрегаты: конверсии объем
    x['A_conv_agg_full'] = x['A_events'] + (8 * x['A_ads']) + (500 * x['A_ipotek']) + (500 * x['A_ct']) + (
                8 * x['A_events_applications']) + (10 * x['A_events_ss'])
